...
text_to_decode  = ARGV[1]

abort('We need at least 2 arguments : 1) shift number 2) text to decode') if ARGV.length < 2
...
def decode_text(new_alphabet, text_to_decode)
# TODO DECODE	
end

new_alphabet = generate_new_alphabet(old_alphabet, shift_nb)

puts 'new_text: ' + decode_text(new_alphabet, text_to_decode)
